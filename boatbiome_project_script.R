...
#Violin plot of alpha diversity Observed OTUs and Shannon Diversity
justbacteria %>%                                                     #phyloseq object
  ...
  geom_text(aes(label = sig, y = 1), vjust = -38.5, size = 6)+
...
#Water + dock + air = sources
wda_source <- read.delim("/home/lgschaer/old/boatbiome/st_input/mixing_prop_wda_source/mixing_proportions.txt")
head(wda_source)
dim(wda_source)

#add sample info to source tracker output
wda_prop <- wda_source %>%
  ...
  gather("Air", "Dock", "Water", "Unknown", key = source, value = proportion)%>%
...
ggplot(data = wda_prop, aes(x = sampletype, y = sumprop, fill = source))+
...
#make coefficient of variation plot
